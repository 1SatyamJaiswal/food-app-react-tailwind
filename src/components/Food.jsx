            ...
            <div className='grid grid-cols-2 lg:grid-cols-4 gap-6 pt-4'>
                {foods.map((item, index) => (
                    <div key={index} className='border shadow-lg'>
                        <img src={item.image} alt={item.name} className='w-full h-[200px] object-cover hover:scale-105 duration-300 rounded-lg'></img>
                        <div className='flex justify-between px-2 py-4'>
                            <p className='font-bold'>{item.name}</p>
                            <p>
                                <span className='bg-orange-600 rounded-lg text-white p-1'>{item.price}</span>
                            </p>
                        </div>
                    </div>
                ))}
